...
if(jQuery) (function($) {
	...
	$.extend($.fn, {
		...
		miniColors: function(custom, o, data) {
			...
			var create = function(input, o, data) {
				...
				// Hide the input that holds the hex value
				input.hide();
			...
			var show = function(input) {
				...
				var selector = $('<div class="miniColors-selector">');
				selector.append('<img class="miniColors-arrow" src="img/color-picker-arrow.png" alt="" />');
				selector.append('<img class="miniColors-close" src="img/small_close_x.png" alt="X" />');
				var customColors = '<div class="miniColors-defaults">';
				for(var i=0;i<custom.length;i++){
					pos = i+1;
					cls = (pos%5 == 0) ? 'class="last"' : '';
					customColors += '<a title="'+custom[i]+'" style="background-color:'+custom[i]+'" '+cls+'></a>';
				}
				customColors += '</div>';
				if(custom.length > 0){ selector.append(customColors); }
				selector.append('<div class="miniColors-colors"><div class="miniColors-colorPicker"></div></div>');
				selector.append('<div class="miniColors-hues"><div class="miniColors-huePicker"></div></div>');
				selector.append('</div>');
				
				selector.css({
					left: input.is(':visible') ? input.offset().left + input.outerWidth() : input.data('trigger').offset().left + input.data('trigger').outerWidth(),
					top: input.is(':visible') ? input.offset().top -5 : input.data('trigger').offset().top -5,
					display: 'none'
				...
				$(document).bind('mousedown.miniColors', function(event) {
					...
					//bind the click of the close button/img
					if(event.target.className == 'miniColors-close'){
						hide(input);
					}					
					
					//bind the default color options
					if( $(event.target).parent().hasClass('miniColors-defaults')){
						var chosenHexColor = $(event.target).attr('title');
						input.val(chosenHexColor);
			...
			var moveColor = function(input, event) {
				...
				if( position.x >= 94 ) position.x = 94;
				if( position.y <= -5 ) position.y = -5;
				if( position.y >= 94 ) position.y = 94;
				input.data('colorPosition', position);
				colorPicker.css('left', position.x).css('top', position.y).show();
				
				// Calculate saturation
				var s = Math.round((position.x + 5) * 1);
				if( s < 0 ) s = 0;
				if( s > 100 ) s = 100;
				
				// Calculate brightness
				var b = 100 - Math.round((position.y + 5) * 1);
				if( b < 0 ) b = 0;
				if( b > 100 ) b = 100;
			...
			var moveHue = function(input, event) {
				...
				if( position.y >= 99 ) position.y = 99;
				input.data('huePosition', position);
				huePicker.css('top', position.y).show();
				
				// Calculate hue
				var h = Math.round((100 - position.y - 1) * 3.6);
				if( h < 0 ) h = 0;
				if( h > 360 ) h = 360;
			...
			var getColorPositionFromHSB = function(hsb) {
				...
				var x = Math.ceil(hsb.s / 1);
				if( x < 0 ) x = 0;
				if( x > 100 ) x = 100;
				
				var y = 100 - Math.ceil(hsb.b / 1);
				if( y < 0 ) y = 0;
				if( y > 100 ) y = 100;
